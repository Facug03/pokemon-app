...
export default function Create () {
  ...
  const handleSubmit = ev => {
    ...
    if (validate) {
      axios.post('https://pokemons-back-production.up.railway.app/pokemons', {
        pokemon: {
          name: form.name,
          hp: Number(form.hp),
    ...
    <section className={styles.container}>
      ...
      <form className={styles.formpoke} onSubmit={handleSubmit}>
        <div className={styles.inputstyle}>
          ...
          <p className={styles.message}>{err.name || (error.includes('name') && error)}</p>
        </div>
        <div className={styles.inputstyle}>
      ...
      {response &&
        <div className={styles.pokecreated}>
          <div className={styles.info}>
            ...
            <a className={styles.accept} href='http://localhost:5173'>Accept</a>
          </div>
        </div>}
      {error.includes('data') &&
        <div className={`${styles.pokecreated} ${!error.includes('data') && styles.inactive}`}>
          <div className={styles.info}>
            <p className={styles.response}>{error}</p>
            <span className={styles.accept} onClick={() => setError('')}>Try again</span>
          </div>
        </div>}
    </section>
  )
...
const validateUrl = url => {
  if (!/[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&//=]*)/.test(url)) return 'invalid url image'
}
